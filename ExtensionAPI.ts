...
export class ExtensionAPI extends ContractAPI {
    ...
    where_HELP() { return `Where is the contract? 
    ...
    deploy_HELP() { return `call near deploy on ${cliConfig.contractAccount} 
    Example: 
    ...
    myfn_HELP() { return `This is a command extension example wiht variable args. 
    ...
    // -----------------------------------------------
    nep21_HELP() { return `Call functions on NEP21 contracts.
    Examples:
    ...
    depo(a /*:CommandLineArgs*/) {
        options.amount.value = a.consumeAmount("amount to deposit", "N") /*+as string+*/
    ...
    info_HELP() { return "get_owner_id, get_staking_key & get_total_staked_balance"}
